...
- Файлы перевода: `translations/ru/{dialogs,ui}/*.ndjson`
- Словарь рантайма: `Third Crisis Neon Nights_Data/Managed/NeonTranslatorRuntime_Data.ndjson` (версионируется)
...
- `node .opencode/skills/translate-batch/merge.mjs` — сборка единого словаря из translations/ru/ → Managed/NeonTranslatorRuntime_Data.ndjson
- `node .opencode/skills/translate-batch/merge.mjs --dry-run` — тестовый прогон мержа
- `node .opencode/skills/translate-batch/batch.test.mjs` — тесты batch (11 тестов)
- `node .opencode/skills/translate-batch/merge.test.mjs` — тесты merge (13 тестов)
...
bundle-parser.mjs → NDJSON (.bundle)     ┼ → extractor.mjs → dialogs/ + ui/ NDJSON
                                        ...
                                        extractor.mjs --merge translations/ru/
                                                   ↓
                                        translate-batch → Google Translate
                                                   ↓
                                         merge.mjs
                                                   ↓
                                         NeonTranslatorRuntime_Data.ndjson
                                                   ↓
                                         build.mjs → copy to Managed/
```
parser.mjs → NDJSON (.assets/level)      ┐
bundle-parser.mjs → NDJSON (.bundle)     ┼ → extractor.mjs → dialogs/ + ui/ NDJSON
                                         ┘
...
## Output structure (generated, не в git)
