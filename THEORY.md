...
Парсер `parser.py` читает **напрямую структурированные Unity-объекты** из бинарников, минуя сырой ASCII-поиск:

1. **Режим `--dialogue`** — извлекает массив `DialogueHistory` из `resources.assets` (structured Speaker + Text + color), выводит NDJSON: `["eng","","speaker"]`
2. **Режим `--texts`** — извлекает UI-строки (TMP_Text, Settings-ключи, названия сцен), выводит NDJSON: `["eng",""]`
3. **Режим `--characters`** — извлекает уникальные имена спикеров, выводит NDJSON: `["eng","","gender"]`
...
В отличие от сырого ASCII-поиска, `parser.py --dialogue` читает **напрямую** сериализованный массив `DialogueHistory` внутри `resources.assets`. Формат записи:
...
> **Текущий парсер `parser.py` читает структурированные данные и не использует сырой ASCII-поиск.** Статистика выше — от старого `parser.mjs` (исторический метод).
...
### 6.5. Оценка покрытия (историческая)

| Метод                      | Всего строк | Чистых диалогов | Доля |
| -------------------------- | ----------- | --------------- | ---- |
| Parser (data section, JS)  | 2 187 546   | —               | —    |
| Extractor (dialogue)       | 24 086      | ~8 000-12 000   | ~40% |
| Extractor (UI)             | 757 971     | 757 971         | 100% |

**Текущий парсер `parser.py --dialogue` даёт 1544 структурированных диалога
без шума — 100% точность. UI-парсер `--texts` даёт 102 UI строки.**

Слабое место исторического метода: фильтр `isDialogue()` слишком широкий — пропускал
...
Словарь загружается из NDJSON-файлов рекурсивно через `TranslationLoader.Load()`.
Формат: `["original","translated","speaker?"]`. Все `*.ndjson` из `translations/`
...
- **диалоги:** 1544 записи (из них ~500 переведено, формат `["eng","rus","speaker"]`)
- **UI:** 102 строки (полностью переведены, формат `["eng","rus"]`)
- **персонажи:** 24 имени (полностью переведены, формат `["eng","rus","gender"]`)
...
translations/
  dialogs/dialogue.ndjson       ← 1544 диалога
  texts/ui.ndjson               ← 102 UI строки
  characters.ndjson             ← 24 персонажа
...
translations/                      ← версионируется в git
  dialogs/dialogue.ndjson          ← диалоги (1544, Speaker + Text + color)
  texts/ui.ndjson                  ← UI строки (102)
  characters.ndjson                ← имена персонажей (24)

Third Crisis Neon Nights_Data/Managed/
  *.ndjson                         ← все ndjson копируются сюда для рантайма
```

### 8.2. Формат NDJSON (перевод)

```ndjson
["I don't know... I murmur, fidgeting.","Я не знаю... — бормочу я, ёрзая.","Zoey"]
...
| --------------- | ----------------------------------------------- |
| `eng`           | Оригинал (английский)                           |
| `rus`           | Перевод (русский), пустая строка = не переведено|
| `speaker`       | Имя персонажа (опционально)                     |

### 8.3. Пайплайн (текущий)

```
Шаг 1: Извлечение диалогов
  parser.py --dialogue → translations/dialogs/dialogue.ndjson
  (Читает DialogueHistory из resources.assets)

Шаг 2: Извлечение UI-текста
  parser.py --texts → translations/texts/ui.ndjson
  (Читает TMP_Text + Settings-ключи из всех бандлов и level)

Шаг 3: Извлечение персонажей
  parser.py --characters → translations/characters.ndjson

Шаг 4: Перевод
  Редактирование ndjson вручную или через translate-expert агента

Шаг 5: Деплой
  build.py → runtime/NeonTranslatorRuntime.dll
  Копирование *.ndjson → Managed/ + dwmapi.dll в корень игры
```
...
| -------------------- | ---------------- | ---------------------------------------------- |
| Парсер бинарников    | Python 3.11+     | struct, os — встроенная работа с бинарными     |
| Извлечение строк     | Pure Python      | Чтение Unity сериализованных объектов          |
...
- Сторонние библиотеки для парсинга Unity
...
Первый подход (бинарный ASCII-поиск через JS `parser.mjs`) давал ~40-60% текста с
умеренным уровнем шума. **Текущий подход** (`parser.py`) читает **структурированные
Unity-объекты**: DialogueHistory для диалогов (100% точность, 0% шума) и TMP_Text
для UI. Это стало возможным благодаря анализу Unity serialized формата и прямой
десериализации MonoBehaviour-блоков.

Ключевые достижения:

- **Структурный парсинг:** чтение DialogueHistory (Speaker + Text) напрямую из `resources.assets`
- **1544 диалога** без шума, 102 UI строки, 24 персонажа
...
- **Python-парсер** вместо Node.js — читает напрямую сериализованные Unity-объекты

**Итоговая архитектура:**

```
parser.py --dialogue (resources.assets → DialogueHistory)
parser.py --texts    (level, .bundle → TMP_Text, Settings)
parser.py --characters (уникальные спикеры)
         │
         ▼
translations/
  dialogs/dialogue.ndjson    ← ["eng","rus","speaker"]
  texts/ui.ndjson            ← ["eng","rus"]
  characters.ndjson          ← ["eng","rus","gender"]
         │
         ▼ (копирование *.ndjson → Managed/)
         │
 ┌────────────────────────────────┐
 │          dwmapi.dll            │
