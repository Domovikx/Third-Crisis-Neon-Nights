...
### 5.1. Null-terminated search (текущий extract.mjs)
...
for each byte in file:
  if printable ASCII:
    ...
    if buffer.length >= MIN_LEN (12) and <= MAX_LEN (500):
      check quality heuristics
      if passes → save as string
    clear buffer
```

**Ограничения:**
- Находит только случайные ASCII-последовательности
- Пропускает length-prefixed строки
- Много шума из шейдеров и бинарных данных

**Фильтрация шума:**
- Минимальная длина: 12 символов
- Буквы > 40% от длины
- Заглавные < 90% от букв
- Хотя бы один пробел (для диалогов)
- Отсев паттернов: пути, шейдеры, base64, хеши
- Отсев по словарю шумовых слов (hotel, room, collider, sprite и т.д.)
- Проверка на наличие распространённых английских слов
- Отсев строк с подчёркиваниями (имена ассетов)

### 5.2. Data section null-terminated search (текущий метод parse-unity)
...
3. for each byte in data section:
     if printable ASCII → build string
       ...
       if string passes quality heuristics → save
       reset buffer
```
...
**Преимущества перед length-prefixed search:**
- Данная игра хранит диалоги как null-terminated (не length-prefixed)
- length-prefixed поиск находит ~40% текста (FsmVariables, object names)
- null-terminated поиск находит ~95% текста
...
| Файл | Размер | version | dataOffset | Диалогов (parse-unity) |
|------|--------|---------|------------|----------------------|
| level0 | 0.11 MB | 22 | — | 37 |
| level1 | 0.06 MB | 22 | — | 8 |
| level2 | 0.13 MB | 22 | — | 13 |
| level3 | 7.03 MB | 22 | 1 097 200 | 1 532 |
| level4 | 1.77 MB | 22 | — | 633 |
| level5 | 1.84 MB | 22 | — | 500 |
| level6 | 1.51 MB | 22 | — | 88 |
| level7 | 12.84 MB | 22 | — | 2 434 |
| level8 | 0.93 MB | 22 | — | 132 |
| level9 | 2.05 MB | 22 | — | 525 |
| level10 | 2.59 MB | 22 | — | 754 |
| level11 | 3.32 MB | 22 | 1 097 200 | 532 |
| level12 | 1.41 MB | 22 | — | 494 |
| level13 | 1.22 MB | 22 | — | 147 |
| level14 | 1.20 MB | 22 | — | 102 |
| level15 | 4.06 MB | 22 | — | 370 |
| sharedassets0 | 34.45 MB | 22 | — | 153 |

**Всего (parse-unity, data section):** ~8 454 кандидатов → ~1 487 диалоговых строк, ~62 UI-строки

### 6.2. Примеры найденных строк

```
10-004 Oh hey there Wow youre hot ehehe 1
10-4. We will keep trying.
10-Exactly~ Now you're getting it~
10-Friend Located
10-Lead the way I wanna watch 1
10-Looking for a good time_
10-My. Look at you, eheheh.
10-So big guy Are you just going to finger my ass 1
10-What do you mean 'it can feel you'_
10. Picking up Item
100-So, what did you want to know about them_
100-Wh- Are you serious, Zoey_ Wait, was that you that made the commotion at the Redlight District_ Fuck, this is not okay!
```

### 6.3. Реальные строки диалогов (примеры из разных уровней)
...
And the drone was recording?
Apparently he's got all kinds of shit.
Are Cypher's boobs out?
```

### 6.4. Проблема разделения диалогов и FSM-логики

Большинство найденных строк — это не диалоги, а имена состояний и
переменных PlayMaker FSM:
- `Are we in progress?` — состояние FSM, не диалог
- `Did talk to Cal already?` — условие FSM
- `Ask about Arm` — триггер FSM
- `Begin preloading the CG` — команда скрипта

Отличить диалог от FSM-логики можно по:
- Наличию спецсимволов: кавычки, скобки, ...
- Слов-маркеров диалога: huh, oh, wow, yeah
- Сокращений: I'm, you're, don't, can't
- Эмоциональной пунктуации: !, ?, ~, ...
- Длинных предложений с союзами и предлогами

### 6.5. Оценка покрытия (актуальная)

| Метод | Найдено строк | Чистых диалогов | Шум |
|-------|--------------|----------------|-----|
| Binary search (весь файл) | ~11K | ~40% | 60% |
| Data section null-term | ~8.5K | ~18% | 82% |
| Data section + фильтр слов | ~1.5K | ~36% | 64% |
...
*Документ создан в рамках анализа локализации Third Crisis Neon Nights (Anduo Games, Unity 2022.3.62f3).*
